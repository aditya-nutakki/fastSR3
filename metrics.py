...
def compute_psnr(x, y):
    ...
    x, y  = x.resize((128, 128)), y.resize((128, 128))
    x, y = numpy.array(x).astype(int), numpy.array(y).astype(int)

    psnr = peak_signal_noise_ratio(y, x, data_range=255)
...
def compute_metrics(generated_path, reference_path):
    ...
    for i, generated_image_path in enumerate(generated_images):
        ...
        # print(generated_image.getextrema(), reference_image.getextrema())
    ...
    print(f"Done for path {generated_path}")
    print(f"found {count} number of photos with < 0 psnr")
...
def generate_images(model, save_path = "/mnt/d/work/datasets/celebA_gen"):
    ...
    for _t in ts:
        _t_path = os.path.join(save_path, f"{lr_sz}_{hr_sz}" ,str(_t))
    ...
    for i, (hr_img, lr_img, img_name) in enumerate(loader):
        ...
        # torchshow.save(hr_img, f"hr_img.jpeg")
        # torchshow.save(lr_img, f"lr_img.jpeg")
...
def dummy(path):
    ...
    for image in images:
        if image.endswith(".jpg.jpg"):
            count += 1
            # print(image)
            old_path, new_path = os.path.join(path, image), os.path.join(path, image[:-4])
...
if __name__ == "__main__":
    ...
    ts = [10, 50, 100, 200, 500, 1000, 2000]
    base_path = "/mnt/d/work/datasets/celebA_gen/16_128"

    for _t in ts:
        
        _path = os.path.join(base_path, str(_t))
        print(f"doing path: {_path}")
        # dummy(_path)        
        compute_metrics(generated_path = _path, reference_path = "/mnt/d/work/datasets/celebA_test")
        print()
    ...
    # ---------------- new metrics ----------------
    # for t = 10, ssim = 0.3253, psnr = 16.08, fid = 33.47; 16_128
    # for t = 50, ssim = 0.3120, psnr = 15.78, fid = 24.06; 16_128
    # for t = 100, ssim = 0.3055, psnr = 15.63, fid = 22.60; 16_128
    # for t = 200, ssim = 0.2999, psnr = 15.55, fid = 22.75; 16_128
    # for t = 500, ssim = 0.2923, psnr = 15.44, fid = 26.43; 16_128
    # for t = 1000, ssim = 0.2893, psnr = 15.39, fid = 27.28; 16_128
    # for t = 2000, ssim = 0.2942, psnr = 15.45, fid = 34.73; 16_128

    # for t = 10, ssim = 0.3964; psnr = 17.24; fid = 22.63; 32_128
    # for t = 50, ssim = 0.3893; psnr = 17.06, fid = 19.82; 32_128
    # for t = 100, ssim = 0.3843; psnr = 16.96, fid = 19.88; 32_128
    # for t = 200, ssim = 0.3865; psnr = 17.00, fid = 20.42; 32_128
    # for t = 500, ssim = 0.3791; psnr = 16.85, fid = 21.77; 32_128
    # for t = 1000, ssim = 0.3760; psnr = 16.80, fid = 22.71; 32_128
    # for t = 2000, ssim = 0.3742; psnr = 16.76, fid = 27.37; 32_128
